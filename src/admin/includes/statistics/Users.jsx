...
export default function Users({ fetching, users, data, listings }) {
  return (
    <main className="flex flex-col gap-2">
      <span className="text-lg font-bold">Users Statistics</span>
      <section className="w-full flex gap-4">
            ...
            <main className="flex flex-col gap-1">
              ...
              <h3 className="font-bold text-2xl font-primary">
                {fetching.type === "user" && fetching.status ? (
                  ...
                  <>
                    {users?.filter((prop) => prop.status == "active").length ||
                      0}
                  </>
                )}
              </h3>
            ...
            <main className="flex flex-col gap-1">
              ...
              <h3 className="font-bold text-2xl font-primary">
                {fetching.type === "user" && fetching.status ? (
                  ...
                  <>
                    {users?.filter((prop) => prop.status == "suspended")
                      .length || 0}
                  </>
                )}
              </h3>
            ...
            <main className="flex flex-col gap-1">
              ...
              <h3 className="font-bold text-2xl font-primary">
                {fetching.type === "user" && fetching.status ? (
                  ...
                  <>
                    {users?.filter((prop) => prop.status == "banned").length ||
                      0}
                  </>
                )}
              </h3>
        ...
        <article className="p-6 rounded-3xl flex flex-col w-full gap-4 bg-white">
          <div className="flex items-center w-full justify-between">
            <span className="text-lg font-bold">Users Breakdown</span>
