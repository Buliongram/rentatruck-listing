    ...
    <section className="flex flex-col gap-5 w-full lg:p-14 p-4">
        ...
        <main className="flex justify-between p-6 border border-zinc-300 rounded-2xl bg-white w-full">
          <section className="flex items-center gap-2">
            <form
              ...
              className="h-14 w-14 lg:h-20 lg:w-20 rounded-full bg-[#f5f5f3] flex items-center justify-center text-primary text-xl lg:text-2xl font-semibold cursor-pointer relative group uppercase"
            ...
            >
              ...
              <span className="h-full w-full absolute top-0 left-0 flex flex-col gap-1 items-center justify-center bg-zinc-500/50 rounded-full text-3xl text-white opacity-0 group-hover:opacity-100 transition-all">
                {isLoading.photo ? (
                  <span className="spinner h-[30px] w-[30px] border-4 border-primary border-b-transparent rounded-full inline-block"></span>
                ) : (
                  <>
                    <FaCamera />
                  </>
          ...
          <Link
            ...
            className="text-xs font-normal cursor-pointer flex items-center gap-2 border rounded-full border-zinc-200 py-1.5 h-max px-3 text-zinc-500/80"
          >
            Edit <FaPencilAlt />
          </Link>
        </main>
      ...
      <article className="flex flex-col gap-4 p-6 border border-zinc-300 rounded-2xl bg-white w-full">
        ...
        <section className="flex items-center justify-between text-xs w-full">
          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">First Name</p>
            <p className=" font-medium capitalize">{user.firstname}</p>
          </main>

          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Last Name</p>
        ...
        <section className="flex items-center justify-between text-xs w-full">
          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Email address</p>
            <p className=" font-medium">{user.email}</p>
          </main>

          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Phone number</p>
        ...
        <section className="flex items-center justify-between text-xs w-full">
          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Account role</p>
            <p className=" font-medium">{user.type}</p>
          </main>

          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Country</p>
      ...
      <article className="flex flex-col gap-4 p-6 border border-zinc-300 rounded-2xl bg-white w-full">
        ...
        <section className="flex items-center justify-between text-xs w-full">
          ...
          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">City / State</p>
            <p className=" font-medium capitalize">{user.country}</p>
          </main>
        ...
        <section className="flex items-center justify-between text-xs w-full">
          <main className="flex flex-col gap-1 w-full">
            <p className="font-normal text-zinc-500">Postal code</p>
